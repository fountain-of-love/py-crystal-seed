...
Project docs are organized under a single maturity-based structure:
...
For the reusable adoption model projects should apply when generated from this template, see:
...
Governance controls should follow a stable-facade model:
- keep local command contracts stable (`make ...`, `tools/check_*.py`)
- keep policy internals in reusable package boundaries and migrate those to shared versioned libraries when stable
- treat dependency bumps as the primary maturity upgrade path for generated projects

Current extraction state:
- governance guardrails are externalized as `guardrails-governance`
- release guardrails are externalized as `guardrails-release`
- architecture guardrails are externalized as `guardrails-architecture`
- operations guardrails are externalized as `guardrails-ops`
- local development copies are maintained under `libs/` in this seed repository only
- derived projects can extend governance locally through `project_governance/hooks.py`
- local hooks are additive only: central guardrails must still pass first

Decluttering rule for the cookiecutter repository:
- avoid adding new standalone policy engines directly under `{{cookiecutter.repo_name}}/tools/`
- prefer reusable implementation in guardrail libraries
- mirror only thin wrappers, config, and docs into the generated-project template
...
- `docs/maturity/governance-auditability/version-evolution-guardrail.md`
